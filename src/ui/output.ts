...
export function printFindings(channel: vscode.OutputChannel, findings: SecurityFinding[]): void {
  channel.clear();
  channel.appendLine("[SP] SecurePatch AI Scan Results");
  channel.appendLine("================================");
  channel.appendLine(`[SP] Findings: ${findings.length}`);
  channel.appendLine("");

  if (findings.length === 0) {
    channel.appendLine("[SP] No findings detected.");
    channel.show(true);
    return;
  }

  for (const finding of findings) {
    channel.appendLine(`[SP] [${finding.severity.toUpperCase()}] ${finding.title}`);
